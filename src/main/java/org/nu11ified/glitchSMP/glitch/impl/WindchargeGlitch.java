...
public class WindchargeGlitch extends Glitch implements Listener {
    ...
    public WindchargeGlitch(GlitchSMP plugin, GlitchSettings.GlitchProfile profile) {
        super(
            GlitchType.WINDCHARGE,
            GlitchType.WINDCHARGE.getName(),
            GlitchType.WINDCHARGE.getDescription(),
            profile.cooldownMillis(),
            profile.durationMillis()
        );
        this.plugin = plugin;
        this.effects = plugin.getGlitchEffects();
        this.projectileKey = new NamespacedKey(plugin, "windcharge_glitch");
        this.damageKey = new NamespacedKey(plugin, "windcharge_glitch_damage");
    ...
    public void onEntityDamage(EntityDamageByEntityEvent event) {
        ...
        int threshold = Math.max(1, plugin.getGlitchSettings().getWindchargePassiveHitThreshold());
        int hits = hitCounters.getOrDefault(attacker.getUniqueId(), 0) + 1;
        if (hits >= threshold) {
            hits = 0;
            fireWindCharge(attacker, plugin.getGlitchSettings().getWindchargePassiveDamage());
    ...
    private void startBarrage(Player player) {
        int count = Math.max(1, plugin.getGlitchSettings().getWindchargeBarrageCount());
        int intervalTicks = Math.max(1, plugin.getGlitchSettings().getWindchargeBarrageIntervalTicks());
        double damage = plugin.getGlitchSettings().getWindchargeBarrageDamage();
        int[] remaining = {count};
